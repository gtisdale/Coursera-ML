...
### Executive Summary
In this discussion I present the development of a random forest model for predicting Human Activity based on acceleromter data.  The final model has an accuracy better than 97%.

The algorithm was developed using by creating intial test algorithms on a 75%-25% training-test data split.  Then three different algorithms - simple tree, random forest and boosted trees - were tried to determine which algorithm was most applicable and which variables were most significant. 
...
To futher improve the accuracy of the model we expand the size of the data set.  The data sets above use the mini data set which includes 10% of the data.  In the analysis below we use 20% of the original data for testing and training.  This improves the accuracy to approximately 97%.
